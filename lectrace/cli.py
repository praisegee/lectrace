...
def main() -> None:
    parser = argparse.ArgumentParser(prog="lectrace", description="Interactive Python lecture tracer")
    ...
    build_p.add_argument("--output", "-o", type=Path, default=Path("_site"), metavar="DIR")
    build_p.add_argument("--no-incremental", action="store_true", help="Rebuild all, ignoring cache")

    serve_p = sub.add_parser("serve", help="Build and serve locally with auto-reload on save")
    ...
    run_p.add_argument("--inspect-all", "-I", action="store_true", help="Capture all local variables")
...
def _validate_py_files(files: list[Path]) -> list[Path] | None:
    ...
    for f in files:
        ...
        if f.name.startswith("_"):
            print(f"Error: {f} looks like a helper file — prefix lecture files without underscore")
            sys.exit(1)
        if not f.exists():
...
def _run(path: Path, inspect_all: bool) -> None:
    ...
    if trace.metadata.error:
        print(f"Warning: raised {trace.metadata.error.exception_type}: {trace.metadata.error.message}")
